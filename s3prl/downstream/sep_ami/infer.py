...
class S3PRLModel(nn.Module):
    ...
    def forward(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        """
        #print("waveforms", waveforms.size())
        assert waveforms.size(1) == 1
        ...
        with torch.no_grad():
            ...
            src_stft = torch.stft(
                (waveforms[:, self.ref_channel, :]).to(device),
                self.n_fft,
            ...
            pred_audios_pad = torch.zeros(bs, num_srcs, waveforms.size(2))
...
def main():
    ...
    dataset = SeparationDatasetUttGroup(args.test_dir, rate=16000, channel=datarc['channel'], normalize=args.normalize)
    ...
    for i, v in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        pred_audios = separation_model(audio.float())
        #print("pred_audios", pred_audios.size())
