...
_sd_module.get_plda = lambda *args, **kwargs: None  # PLDA only needed for VBxClustering; skip HuggingFace download
...
class AudioCustom(Audio):
    ...
    def downmix_and_resample(self, waveform: Tensor, sample_rate: int, channel=None) -> Tensor:
        ...
        channel : int, optional
            Ignored — channel selection is controlled by self.mono instead.
...
class S3PRLModel(Model):
    ...
    def receptive_field(self) -> SlidingWindow:
        # step = frame shift of the SSL upstream in seconds
        # duration = step (true CNN receptive field is larger but unused here
        # because specifications.permutation_invariant=True overrides it)
        step = self.featurizer.model.downsample_rate / self.hparams.sample_rate
        return SlidingWindow(start=0.0, duration=step, step=step)
...
def main():
    ...
    if not args.gt_spk_assign:
        ...
        # OracleClustering skips creating _embedding in __init__, but get_embeddings
        # still accesses it unconditionally. Provide a stub and skip embedding extraction
        # since OracleClustering handles embeddings=None and only RTTM output is needed.
        class _EmbeddingStub:
            dimension = 1
        pipeline._embedding = _EmbeddingStub()
        pipeline.get_embeddings = types.MethodType(
            lambda self, *args, **kwargs: None, pipeline
        )
