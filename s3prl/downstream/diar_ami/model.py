...
class Model(nn.Module):
    def __init__(self, input_dim, output_class_num, rnn_layers, hidden_size, bidirectional=False, **kwargs):
        ...
        if self.use_rnn:
            self.rnn = nn.LSTM(
                input_dim, hidden_size, num_layers=rnn_layers, batch_first=True, bidirectional=bidirectional
            )
            if bidirectional:
                self.linear = nn.Linear(hidden_size * 2, output_class_num)
            else:
