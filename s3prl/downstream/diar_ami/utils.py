...
def create_length_mask(length, max_len, num_output):
...
def pit_loss_single_permute(output, label, length):
    ...
    loss = bce_loss(output, label) # [Batch_size, T, Num_spks]
    loss = loss * mask.to(output.device)
...
def pit_loss(output, label, length):
    ...
    loss = torch.mean(min_loss / length.float().to(device))
...
def calc_diarization_error(pred, label, length):
    ...
    label_np = label.float().data.cpu().numpy().astype(int)
    pred_np = (pred.float().data.cpu().numpy() > 0).astype(int)
