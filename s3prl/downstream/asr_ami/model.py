...
def create_padding_mask(x_len):
    max_src_len = torch.max(x_len)

    src_key_padding_mask = torch.zeros((len(x_len), max_src_len), dtype=torch.bool)

    for i, src_len in enumerate(x_len):
        src_key_padding_mask[i, src_len:] = 1
...
class Transformers(nn.Module):
    def __init__(self,
        ...
        self.transformers = nn.ModuleList(
                [TransformerEncoderLayer(d_model, nhead, dim_feedforward=dim_feedforward, dropout=dropout, activation=activation, batch_first=True) for i in range(n_layers)]
            )
        self.output_fc = nn.Linear(d_model, output_size)

    def forward(self, x, x_len):
        src_key_padding_mask = (create_padding_mask(x_len)).to(x.device)
        for tf_module in self.transformers:
            x = tf_module(x, src_key_padding_mask=src_key_padding_mask)
        x = self.output_fc(x)
        return x, x_len
