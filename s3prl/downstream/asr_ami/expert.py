...
class DownstreamExpert(nn.Module):
    ...
    def set_upstream_preprocess(self, batch_fn):
        """Called by the runner to delegate per-sample fbank computation to DataLoader workers."""
        self._wav_preprocess = lambda wav: batch_fn([wav])[0]
    ...
    def get_dataloader(self, split):
        ...
        Args:
            split: string
                The name of the dataloader, can be train/dev/test for asr
        ...
        if not hasattr(self, f'{split}_dataset'):
            setattr(self, f'{split}_dataset', ASRDataset(
                split,
                self.loaderrc[f'{split}_dir'],
                self.dictionary,
                wav_preprocess=getattr(self, '_wav_preprocess', None),
                **self.datarc,
            ))
